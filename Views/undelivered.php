   ...
   <?php include './components/header.php' ; ?>
    <main>
        <div class="container">
          ...
          <?php 
           $status="undelivered";
          $stmt=$orderInstance->orderStatus($status);
          ...
          $totalPages = ceil ($number_of_result / $results_per_page);  
           ...
           $sql="SELECT * FROM orders WHERE order_status=:status ORDER BY id ASC LIMIT ". $startFrom . ',' . $results_per_page;
        ...
        <h5>Note: only orders that has been paid for, but yet to be delivered are listed here,Deliver cart items to customer and mark them as delivered if delivered successfully </h5>
...
<div class="table-container">
  <table>
    ...
    <form action="" class="order-modify">    
      <td> <button class="markbtn" id="eachmark<?php echo  "{$orders['order_id']}" ; ?>" data-identity="<?php echo  "{$orders['order_id']}" ; ?>" >Mark as Delivered</button> <button data-identity="<?php echo  "{$orders['order_id']}" ; ?>" class="deletebtn">Delete</button> </td>  
    </form>    
  ...
  <div class="next-prev">
      <div class="move-button">
         <?php if($page>=2): ?> 
           <a href='undelivered.php?page=<?php echo ($page-1); ?>'>  Prev </a> 
         <?php endif ?>  
         
         <?php for($i = 1; $i<= $totalPages; $i++): ?> 
             <?php if($i == $page): ?>
               <a href ="undelivered.php?page=<?php echo $i; ?>" class ="active"> <?php echo $i ; ?> </a> 
             <?php else: ?>
                <a href ="undelivered.php?page=<?php echo $i; ?>" > <?php echo $i ; ?> </a> 
             <?php endif ?>   
         <?php endfor ?>

         <?php if($page<$totalPages): ?>   
            <a href='undelivered.php?page=<?php echo ($page+1); ?>'>  Next </a>  
         <?php  endif ?>
      </div>
         <div class="inline">   
    ...
    <?php else: ?>
        <h4>No undelivered orders at the moment</h4>
    <?php endif ?>
        </div>
           ...
           <script>
              function go2Page() {   
                  ...
                  window.location.href = 'undelivered.php?page='+page;   
                }   
